...
def writing():
    while True:
        ...
        msg = msg.decode()
        if msg.lower() == "incorrect password":
            print("Couldn't connect to the server due to Incorrect password")
            print("Enter quit to close the socket")
            break
        if msg[:4] == "file":
            div = msg.index(",")
            filename = msg[4:div]
...
if __name__ == '__main__':
    ...
    print('Connecting with ', server_name)
    # message = "get_list"
